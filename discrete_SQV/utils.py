...
class AtariBuffer(object):
	...
	def sample_augmented(self, sampletype="normal"):
		if sampletype == "normal":
			ind = np.random.randint(0, self.crt_size, size=self.batch_size)
		elif sampletype == "10":
			ind = np.random.randint(0, int(self.crt_size / 10), size=self.batch_size)
			ind = ind * 10
		elif sampletype == "1":
			ind = np.random.randint(0, int(self.crt_size / 100), size=self.batch_size)
			ind = ind * 100
		...
		state = torch.ByteTensor(state).to(self.device).float()
		next_state = torch.ByteTensor(next_state).to(self.device).float()
		state_aug = torch.ByteTensor(state_aug).to(self.device).float()
		next_state_aug = torch.ByteTensor(next_state_aug).to(self.device).float()

		state, _ = self.aug_trans(state)
		next_state, _ = self.aug_trans(next_state)
		state_aug, _ = self.aug_trans(state_aug)
		next_state_aug, _ = self.aug_trans(next_state_aug)
		...
		return (
			...
			torch.FloatTensor(self.reward[ind]).to(self.device),
			torch.FloatTensor(self.not_done[ind]).to(self.device),
			state_aug.to(self.device),
			next_state_aug.to(self.device),
		)
...
class AtariPreprocessing(object):
	...
	def aug_trans2(self, imgs, out=84):
		imgs = nn.ReplicationPad2d(self.image_pad)(imgs)
		n, c, h, w = imgs.shape
		crop_max = h - out + 1
		w1 = torch.randint(0, crop_max, (n,))
