...
# Trains BCQ offline
def train_SQV(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters):
	# For saving files
	setting = f"{args.env}_{args.seed}"
	buffer_name = f"{args.buffer_name}_{setting}"

	# Initialize and load policy
	policy = discrete_SQV.discrete_SQV(
		is_atari,
		num_actions,
		state_dim,
		device,
...
if __name__ == "__main__":
	...
	parser.add_argument("--sqv", action="store_true")  # If true, generate buffer
	...
	if args.train_behavioral or args.generate_buffer:
		...
		if not args.sqv:
			print("----- Train as normal BCQ -----")
			train_BCQ(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters)
		else:
			print("----- Train as SQV  -----")
			train_SQV(env, replay_buffer, is_atari, num_actions, state_dim, device, args, parameters)