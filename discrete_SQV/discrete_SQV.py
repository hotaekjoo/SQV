...
class discrete_SQV(object):
    def __init__(
        ...
        self.Q = Conv_Q(state_dim[0], num_actions).to(self.device) if is_atari else FC_Q(state_dim, num_actions).to(
            self.device)
    ...
    def train(self, replay_buffer):
        ...
        state, action, next_state, reward, done, state_aug, next_state_aug = replay_buffer.sample_augmented()
